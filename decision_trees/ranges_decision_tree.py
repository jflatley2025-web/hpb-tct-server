...
def phase9_set_targets(inputs: RangeInputs, result: RangeEvaluation):
    ...
    if result.trade_bias == TradeBias.SHORT:
        ...
        result.passed_phases.append("Phase 9: Short targets set")
    elif result.trade_bias == TradeBias.LONG:
        result.primary_target = "Range High"
        result.minimum_target = "Upper portion (above EQ)"
        result.entry_note = f"Accumulation setup via TCT schematic. {note}"
        result.passed_phases.append("Phase 9: Long targets set")
    else:
        result.passed_phases.append("Phase 9: No targets — waiting for bias")
