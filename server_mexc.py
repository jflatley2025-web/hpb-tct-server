...
# PAUSED: chromadb + SentenceTransformer imports disabled — loading all-MiniLM-L6-v2 at startup
# was consuming >512MB RAM and crashing the Render deployment before the port could open.
# Re-enable when upgrading to a higher-memory instance or moving PDF indexing to a separate service.
...
# PAUSED: ChromaDB client + SentenceTransformer model disabled to reduce memory usage at startup.
# All three objects (embedding_function, chroma_client, collection) are only used by the
# /index-pdf and /index-pdf-folder endpoints below, which are also paused.
# Re-enable by restoring the imports above and uncommenting the block below.
...
# PAUSED: PDF helper functions and /index-pdf endpoints disabled along with ChromaDB above.
# Re-enable by restoring the chromadb imports and the block below.
#
...
async def startup_event():
    ...
    # Start the Engulfing Candle bot auto-scan loop (Phemex simulated trading)
    asyncio.create_task(_auto_scan_supervisor(engulfing_bot_auto_scan_loop, "ENGULFING-BOT"))
    logger.info("[ENGULFING-BOT] Background auto-scan loop launched (supervised)")
...
# ================================================================
# SMA BOT — API ENDPOINTS
# ================================================================

@app.get("/api/sma-bot/state")
async def sma_bot_state():
    """Return current SMA bot state for the dashboard."""
    bot = get_sma_bot()
    return convert_numpy_types(bot.snapshot())


@app.get("/api/sma-bot/scan")
async def sma_bot_scan():
    """Run a single SMA bot scan cycle manually."""
    loop = asyncio.get_event_loop()
    result = await loop.run_in_executor(None, sma_scan_cycle)
    return convert_numpy_types(result)


@app.get("/sma-bot", response_class=HTMLResponse)
async def sma_bot_dashboard():
    """SMA Bot Simulated Trading Dashboard — live view of trades."""
    html = """<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>SMA Bot — Simulated Trading</title>
<style>
    body { background: #0a0a12; color: #e0e0e0; font-family: 'Segoe UI', sans-serif; margin: 0; padding: 20px; }
...
<body>
    ...
    <h1>SMA Bot — Simulated Trading</h1>
    <div class="subtitle">Phemex uBTCUSD | Daily 20-SMA Bias + 15m Entry | All trades simulated</div>
    ...
    <div class="grid">
        ...
        <div class="card">
            ...
            <div id="state_info">
                ...
                <div class="stat"><span class="stat-label">Last Action</span><span class="stat-value" id="last_action">—</span></div>
                <div class="stat"><span class="stat-label">Last Scan</span><span class="stat-value" id="last_scan">—</span></div>
            </div>
        </div>
    </div>
...
async function fetchState() {
    try {
        const res = await fetch('/api/sma-bot/state');
        const s = await res.json();
        ...
        document.getElementById('starting').textContent = '$' + s.starting_balance.toFixed(2);
        const pnlEl = document.getElementById('pnl');
        pnlEl.textContent = '$' + s.pnl_total.toFixed(2);
        pnlEl.className = 'stat-value ' + (s.pnl_total >= 0 ? 'positive' : 'negative');
        const pctEl = document.getElementById('pnl_pct');
        pctEl.textContent = s.pnl_pct.toFixed(2) + '%';
        pctEl.className = 'stat-value ' + (s.pnl_pct >= 0 ? 'positive' : 'negative');
        ...
        console.error('Failed to fetch SMA bot state:', e);
    }
}
...
async function manualScan() {
    ...
    status.textContent = 'Scanning...';
    try {
        const res = await fetch('/api/sma-bot/scan');
        const r = await res.json();
        status.textContent = 'Done: ' + (r.action || 'ok');
        fetchState();
    } catch(e) {
...
# ================================================================
# SMA BOT — BACKGROUND AUTO-SCAN LOOP
# ================================================================
...
async def sma_bot_auto_scan_loop():
    ...
    while True:
        try:
            loop = asyncio.get_event_loop()
            result = await asyncio.wait_for(
                loop.run_in_executor(None, sma_scan_cycle),
                timeout=300,
            )
            action = result.get("action", "unknown")
            logger.info(f"[SMA-BOT] Scan result: {action}")
            consecutive_errors = 0
        except Exception as e:
            consecutive_errors += 1
            logger.error(f"[SMA-BOT] Scan error ({consecutive_errors}): {e}", exc_info=True)
            if consecutive_errors >= 5:
...
# ================================================================
# ENGULFING BOT — API ENDPOINTS
# ================================================================

@app.get("/api/engulfing-bot/state")
async def engulfing_bot_state():
    """Return current Engulfing bot state for the dashboard."""
    bot = get_engulfing_bot()
    return convert_numpy_types(bot.snapshot())


@app.get("/api/engulfing-bot/scan")
async def engulfing_bot_scan():
    """Run a single Engulfing bot scan cycle manually."""
    loop = asyncio.get_event_loop()
    result = await loop.run_in_executor(None, engulfing_scan_cycle)
    return convert_numpy_types(result)


@app.get("/engulfing-bot", response_class=HTMLResponse)
async def engulfing_bot_dashboard():
    """Engulfing Candle Bot Simulated Trading Dashboard — live view of trades."""
    html = """<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Engulfing Bot — Simulated Trading</title>
<style>
    body { background: #0a0a12; color: #e0e0e0; font-family: 'Segoe UI', sans-serif; margin: 0; padding: 20px; }
    h1 { color: #ff9800; font-size: 1.4rem; margin-bottom: 4px; }
    .subtitle { color: #888; font-size: 0.85rem; margin-bottom: 20px; }
    .nav-links { display: flex; gap: 16px; margin-bottom: 20px; }
    .nav-links a { color: #00d4ff; text-decoration: none; font-size: 0.85rem; }
    .grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 16px; margin-bottom: 20px; }
    .card { background: #12121a; border: 1px solid #1e1e2d; border-radius: 8px; padding: 16px; }
    .card h2 { font-size: 1rem; color: #aaa; margin: 0 0 12px 0; }
    .stat { display: flex; justify-content: space-between; padding: 6px 0; border-bottom: 1px solid #1a1a2a; }
    ...
    .badge-buy { background: rgba(0,212,255,0.15); color: #00d4ff; }
    .badge-sell { background: rgba(224,64,251,0.15); color: #e040fb; }
    .badge-bullish { background: rgba(0,255,136,0.15); color: #00ff88; }
    .badge-bearish { background: rgba(255,68,68,0.15); color: #ff4444; }
    .badge-none { background: rgba(136,136,136,0.15); color: #888; }
    .no-data { color: #555; text-align: center; padding: 20px; }
    .auto-refresh { color: #555; font-size: 0.75rem; text-align: right; }
    .pending-order { background: rgba(255,193,7,0.08); }
    .action-bar { margin-bottom: 16px; display: flex; gap: 12px; align-items: center; }
    ...
    .btn:hover { background: #2d2d44; }
    .btn:disabled { opacity: 0.5; cursor: not-allowed; }
    .engulf-indicator { display: flex; align-items: center; gap: 8px; padding: 6px 0; border-bottom: 1px solid #1a1a2a; }
    .engulf-indicator:last-child { border-bottom: none; }
    .engulf-label { color: #888; flex: 1; }
    .sma-bar { display: flex; align-items: center; gap: 8px; padding: 4px 0; }
    .sma-label { font-size: 0.75rem; color: #888; width: 70px; }
    .sma-line { flex: 1; height: 2px; border-radius: 1px; background: #ff9800; }
    .sma-price { font-size: 0.85rem; font-weight: 600; min-width: 90px; text-align: right; color: #ff9800; }
</style>
</head>
<body>
    <div class="nav-links">
        <a href="/">← Dashboard</a>
        <a href="/sma-bot">SMA Bot</a>
        <a href="/breakout-bot">Breakout Bot</a>
    </div>
    <h1>Engulfing Candle Bot — Simulated Trading</h1>
    <div class="subtitle">Phemex BTC/USD:BTC | 15m Engulfing Candle + 20-SMA Filter | All trades simulated</div>
    ...
    <div class="grid">
        ...
        <div class="card">
            <h2>Signal & State</h2>
            <div class="engulf-indicator">
                <span class="engulf-label">Engulf Signal</span>
                <span id="signal_badge">—</span>
            </div>
            <div class="sma-bar">
                <span class="sma-label">SMA20 15m</span>
                <div class="sma-line"></div>
                <span class="sma-price" id="sma20">—</span>
            </div>
            <div class="stat"><span class="stat-label">Last Bid</span><span class="stat-value" id="last_bid" style="color:#ffc107;">—</span></div>
            <div class="stat"><span class="stat-label">Last Action</span><span class="stat-value" id="last_action" style="font-size:0.8rem;">—</span></div>
            <div class="stat"><span class="stat-label">Last Scan</span><span class="stat-value" id="last_scan">—</span></div>
        </div>
    </div>
...
async function fetchState() {
    try {
        const res = await fetch('/api/engulfing-bot/state');
        const s = await res.json();
        ...
        const sigEl = document.getElementById('signal_badge');
        if (sig === 'BULLISH_ENGULF') {
            sigEl.innerHTML = '<span class="badge badge-bullish">BULLISH ENGULF</span>';
        } else if (sig === 'BEARISH_ENGULF') {
            sigEl.innerHTML = '<span class="badge badge-bearish">BEARISH ENGULF</span>';
        } else {
            sigEl.innerHTML = '<span class="badge badge-none">NO SIGNAL</span>';
        }

        document.getElementById('sma20').textContent = s.last_sma20 ? '$' + s.last_sma20.toFixed(2) : '—';
        document.getElementById('last_bid').textContent = s.last_bid ? '$' + s.last_bid.toFixed(2) : '—';
        document.getElementById('last_action').textContent = s.last_cycle_action || '—';
        document.getElementById('last_scan').textContent = s.last_cycle_time ? new Date(s.last_cycle_time).toLocaleTimeString() : '—';
        ...
        console.error('Failed to fetch engulfing bot state:', e);
    }
}
...
async function manualScan() {
    ...
    status.textContent = 'Scanning...';
    try {
        const res = await fetch('/api/engulfing-bot/scan');
        const r = await res.json();
        status.textContent = 'Done: ' + (r.action || 'ok');
        fetchState();
    } catch(e) {
...
# ================================================================
# ENGULFING BOT — BACKGROUND AUTO-SCAN LOOP
# ================================================================
...
async def engulfing_bot_auto_scan_loop():
    ...
    while True:
        try:
            loop = asyncio.get_event_loop()
            result = await asyncio.wait_for(
                loop.run_in_executor(None, engulfing_scan_cycle),
                timeout=300,
            )
            action = result.get("action", "unknown")
            logger.info(f"[ENGULFING-BOT] Scan result: {action}")
            consecutive_errors = 0
        except Exception as e:
            consecutive_errors += 1
            logger.error(f"[ENGULFING-BOT] Scan error ({consecutive_errors}): {e}", exc_info=True)
            if consecutive_errors >= 5:
...
# ================================================================
# TURTLE BOT — API ENDPOINTS
# ================================================================

@app.get("/api/turtle-bot/state")
async def turtle_bot_state():
    """Return current Turtle bot state for the dashboard."""
    bot = get_turtle_bot()
    return convert_numpy_types(bot.snapshot())


@app.get("/api/turtle-bot/scan")
async def turtle_bot_scan():
    """Run a single Turtle bot scan cycle manually."""
    loop = asyncio.get_event_loop()
    result = await loop.run_in_executor(None, turtle_scan_cycle)
    return convert_numpy_types(result)


@app.get("/turtle-bot", response_class=HTMLResponse)
async def turtle_bot_dashboard():
    """Turtle Strategy Bot Simulated Trading Dashboard — live view of trades."""
    html = """<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Turtle Bot — Simulated Trading</title>
<style>
    body { background: #0a0a12; color: #e0e0e0; font-family: 'Segoe UI', sans-serif; margin: 0; padding: 20px; }
    h1 { color: #00bcd4; font-size: 1.4rem; margin-bottom: 4px; }
    .subtitle { color: #888; font-size: 0.85rem; margin-bottom: 20px; }
    .nav-links { display: flex; gap: 16px; margin-bottom: 20px; }
    .nav-links a { color: #00d4ff; text-decoration: none; font-size: 0.85rem; }
    .grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 16px; margin-bottom: 20px; }
    ...
    .badge-buy { background: rgba(0,212,255,0.15); color: #00d4ff; }
    .badge-sell { background: rgba(224,64,251,0.15); color: #e040fb; }
    .badge-active { background: rgba(0,188,212,0.15); color: #00bcd4; }
    .badge-inactive { background: rgba(136,136,136,0.15); color: #888; }
    .no-data { color: #555; text-align: center; padding: 20px; }
    .auto-refresh { color: #555; font-size: 0.75rem; text-align: right; }
    .pending-order { background: rgba(255,193,7,0.08); }
    .action-bar { margin-bottom: 16px; display: flex; gap: 12px; align-items: center; }
    ...
    .btn:hover { background: #2d2d44; }
    .btn:disabled { opacity: 0.5; cursor: not-allowed; }
    .level-bar { display: flex; align-items: center; gap: 8px; padding: 4px 0; border-bottom: 1px solid #1a1a2a; }
    .level-bar:last-child { border-bottom: none; }
    .level-label { font-size: 0.75rem; color: #888; width: 80px; }
    .level-line { flex: 1; height: 2px; border-radius: 1px; }
    .level-price { font-size: 0.85rem; font-weight: 600; min-width: 100px; text-align: right; }
    .high-color { background: #00ff88; color: #00ff88; }
    .bid-color  { background: #ffc107; color: #ffc107; }
    .low-color  { background: #ff4444; color: #ff4444; }
    .tp-color   { background: #00bcd4; color: #00bcd4; }
    .sl-color   { background: #ff9800; color: #ff9800; }
    .atr-color  { color: #aaa; }
</style>
</head>
<body>
    <div class="nav-links">
        ...
        <a href="/sma-bot">SMA Bot</a>
        <a href="/breakout-bot">Breakout Bot</a>
        <a href="/engulfing-bot">Engulfing Bot</a>
    </div>
    <h1>Turtle Strategy Bot — Simulated Trading</h1>
    <div class="subtitle">Phemex BTC/USD:BTC | 55-Bar Breakout | TP 0.2% | SL 2×ATR | Mon–Fri 9:30–16:00 ET | All trades simulated</div>
    ...
    <div class="grid">
        ...
        <div class="card">
            <h2>Levels & State</h2>
            <div class="level-bar"><span class="level-label">55-Bar High</span><div class="level-line high-color"></div><span class="level-price high-color" id="high55">—</span></div>
            <div class="level-bar"><span class="level-label">Bid</span><div class="level-line bid-color"></div><span class="level-price bid-color" id="last_bid">—</span></div>
            <div class="level-bar"><span class="level-label">55-Bar Low</span><div class="level-line low-color"></div><span class="level-price low-color" id="low55">—</span></div>
            <div class="level-bar"><span class="level-label">ATR (14)</span><div class="level-line" style="background:#555;"></div><span class="level-price atr-color" id="atr">—</span></div>
            <div class="stat" style="margin-top:8px;"><span class="stat-label">Market Hours</span><span id="hours_badge">—</span></div>
            <div class="stat"><span class="stat-label">Timeframe</span><span class="stat-value" id="timeframe">—</span></div>
        </div>
    </div>
    ...
    <div class="card" style="margin-bottom: 16px;">
        <h2>Last Action</h2>
        <div class="stat"><span class="stat-label">Action</span><span class="stat-value" id="last_action" style="font-size:0.8rem;word-break:break-all;">—</span></div>
        <div class="stat"><span class="stat-label">Last Scan</span><span class="stat-value" id="last_scan">—</span></div>
    </div>
...
async function fetchState() {
    try {
        const res = await fetch('/api/turtle-bot/state');
        const s = await res.json();
        ...
        document.getElementById('high55').textContent = s.last_55bar_high ? '$' + s.last_55bar_high.toFixed(2) : '—';
        document.getElementById('last_bid').textContent = s.last_bid ? '$' + s.last_bid.toFixed(2) : '—';
        document.getElementById('low55').textContent = s.last_55bar_low ? '$' + s.last_55bar_low.toFixed(2) : '—';
        document.getElementById('atr').textContent = s.last_atr ? s.last_atr.toFixed(2) : '—';
        document.getElementById('timeframe').textContent = s.config?.timeframe || '—';

        const hoursEl = document.getElementById('hours_badge');
        if (s.in_trading_hours) {
            hoursEl.innerHTML = '<span class="badge badge-active">MARKET OPEN</span>';
        } else {
            hoursEl.innerHTML = '<span class="badge badge-inactive">MARKET CLOSED</span>';
        }
        ...
        if (s.current_position) {
            const p = s.current_position;
            const sideBadge = p.side === 'long' ? 'badge-long' : 'badge-short';
            const tpHtml = s.last_tp_price
                ? `<div class="stat"><span class="stat-label">Take Profit</span><span class="level-price tp-color">$${s.last_tp_price.toFixed(2)}</span></div>`
                : '';
            const slHtml = s.last_sl_price
                ? `<div class="stat"><span class="stat-label">Stop Loss (2×ATR)</span><span class="level-price sl-color">$${s.last_sl_price.toFixed(2)}</span></div>`
                : '';
            posDiv.innerHTML = `
                <div class="stat"><span class="stat-label">Side</span><span class="badge ${sideBadge}">${p.side.toUpperCase()}</span></div>
                <div class="stat"><span class="stat-label">Size</span><span class="stat-value">${p.size}</span></div>
            ...
            posDiv.innerHTML = '<div class="no-data">No open position — scanning every ' + (s.config?.scan_interval || 60) + 's during market hours</div>';
        }
        ...
        console.error('Failed to fetch turtle bot state:', e);
    }
}
...
async function manualScan() {
    ...
    status.textContent = 'Scanning...';
    try {
        const res = await fetch('/api/turtle-bot/scan');
        const r = await res.json();
        status.textContent = 'Done: ' + (r.action || 'ok');
        fetchState();
    } catch(e) {
...
# ================================================================
# TURTLE BOT — BACKGROUND AUTO-SCAN LOOP
# ================================================================
...
async def turtle_bot_auto_scan_loop():
    ...
    while True:
        try:
            loop = asyncio.get_event_loop()
            result = await asyncio.wait_for(
                loop.run_in_executor(None, turtle_scan_cycle),
                timeout=300,
            )
            action = result.get("action", "unknown")
            logger.info(f"[TURTLE-BOT] Scan result: {action}")
            consecutive_errors = 0
        except Exception as e:
            consecutive_errors += 1
            logger.error(f"[TURTLE-BOT] Scan error ({consecutive_errors}): {e}", exc_info=True)
            if consecutive_errors >= 5:
